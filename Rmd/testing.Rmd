...
# CRD - 3 x 2 to compare model SE vs sample SE
...
# fig7d-CRD-NB-LSD1 inhibition circumvents glucocorticoid-induced muscle wasting of male mice

```{r}
data_from <- "data from"
data_folder <- "fig7d-CRD-NB-LSD1 inhibition circumvents glucocorticoid-induced muscle wasting of male mice"
file_path <- here(data_from, data_folder, "fig7d-CRD-NB-LSD1 inhibition circumvents glucocorticoid-induced muscle wasting of male mice.xlsx")
fig7d <- read_xlsx(file_path) |>
    data.table()
...
m1 <- lm(fibrotic_area_proportion ~ genotype,
          data = fig2d)
m1_emm <- emmeans(m1, specs = "genotype", type = "response")
m1_pairs <- contrast(m1_emm, method = "revpairwise") |>
  summary(infer = TRUE)
