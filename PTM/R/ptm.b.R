    ...
    .run = function() {
      ...
      if(working_model == TRUE){
        ...
        if(include_nest == FALSE){
          ...
          plotData_full[, nest_id := self$data[[self$options$nest_id]]]
        }
        ...
        # create dataset for m1_check
        if(include_nest == TRUE){
          new_data[, y := y_hat]
          setnames(new_data, old = "y", new = response_label)
          #              data_check <- copy(new_data)
        }
...
#      }
    ...
    .plot=function(image, ...) {
      ...
      if(!is.null(plotData)){
        ...
        # tech replicates
        show_tech_reps <- ifelse(self$options$tech_reps == TRUE, TRUE, FALSE)
        if(is.null(self$options$nest_id)){show_tech_reps <- FALSE}
